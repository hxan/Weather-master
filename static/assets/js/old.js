...
function show_data(page_data, page_no, page_list) {
    ...
    for (var i = 0; i < page_data.length; i++) {
        ...
        list_data = list_data + '<tr>' +
            ...
            '<td>' + item[9] + '</td>' +
            '<td class="td-manage">' +
            ' <a title="编辑"  onclick="x_old_edit(\'编辑\',\'' + item[0] + '\',' + item[1] + ',' + item[2] + ',' + item[3] + ',' + item[4] + ',' + item[5] + ',' + item[6] + ',' + item[7] + ',' + item[8] + ','+ item[9] + ')" href="javascript:;">' +
...
function x_old_edit(title, a, Date, b, c, d, e, f, g, h, i, w, he) {
    if (w == null || w == '') {
        w = 600
    }
    if (he == null || he == '') {
...
function get_search() {
    ...
    console.log(start.slice(-2))     //只能搜索三月下旬的气象
    if (start != null && start != '') {
        param = param + " and Hour=23 and Day>= '" + start.slice(-2) + "'";
    }
    if (end != null && end != '') {
        param = param + " and Hour=23 and Day<= '" + end.slice(-2) + " 23:59:59'";
    }
